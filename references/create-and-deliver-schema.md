...
| `cardData.cardParamMap` | 是 | 模板变量映射；所有变量值均为字符串；key 不超过 100 UTF-8 字节，value 不超过 1024 UTF-8 字节 |
...
| 表单变量 | `../assets/feedback-form.schema.json` | `feedbackForm` 反序列化后的对象 |
| 满意度变量 | `../assets/satisfaction-options.schema.json` | `satisfactionOptions` 反序列化后的数组 |

所有 Schema 使用 JSON Schema Draft 2020-12，由 `jsonschema>=4.18,<5` 执行。依赖版本见 `../scripts/requirements.txt`。此外，Python 校验器只负责 JSON Schema 难以表达的跨字段关系、递归敏感字段扫描和 UTF-8 字节长度。
...
禁止直接手写 `feedbackForm` 或 `satisfactionOptions` 字符串。真实发送不得使用校验器的 `--skip-env-check`。
