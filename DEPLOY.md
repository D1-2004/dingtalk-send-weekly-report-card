...
- 已读报文的 `row` 包含基础信息、「是否已读=是」、「读取人ID」（钉钉 `uid`）和「读取人昵称」，在页面 `load` 事件触发后等待身份读取并发送；身份读取失败时降级为 `anonymous / 匿名客户`。协议见 `assets/weekly-feedback-read.schema.json`。
- 反馈自动化的原生循环直接消费 Webhook 的 `rows`；已读自动化直接引用 Webhook 的 `row`，两者都不使用 Python 或其他脚本解析节点。
- 两条自动化都按「编号 = `outTrackId`」查找后 upsert：反馈流程仅更新基础信息与反馈字段；已读流程仅更新基础信息、「是否已读」、「读取人ID」和「读取人昵称」。这样无论两个请求的到达顺序如何，都只保留同一行并避免互相清空字段。
...
- 反馈页在钉钉容器内读取当前用户身份用于实名提交和已读上报；身份读取失败（浏览器/外部客户）时降级为 `anonymous / 匿名客户`，不硬拦。

## 6. 协议变更记录

- 2026-09-07：已读上报新增「读取人ID」和「读取人昵称」，并复用反馈页已有的钉钉身份读取能力。原因：底表需要记录实际打开周报页面的人员，支持按用户核对阅读情况。
