...
class SimplifiedSkillTests(unittest.TestCase):
    ...
    def test_read_schema_combines_base_data_with_read_status(self) -> None:
        ...
        self.assertEqual(row["properties"]["读取人ID"]["minLength"], 1)
        self.assertEqual(row["properties"]["读取人昵称"]["minLength"], 1)
        self.assertIn("读取人ID", row["required"])
        self.assertIn("读取人昵称", row["required"])
...
class HtmlGenerationTests(unittest.TestCase):
    def test_html_injects_submit_url_and_returns_output(self) -> None:
        ...
        self.assertIn('"读取人ID": reader.userId', generated_runtime)
        self.assertIn('"读取人昵称": reader.name || reader.userId', generated_runtime)
        self.assertIn("reader = await getFeedbackUser()", generated_runtime)
