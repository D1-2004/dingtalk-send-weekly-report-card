...
1. 用周报里的项目名 / 客户名 / 项目编号在底表匹配到具体项目行（用当前 Agent 已有的钉钉 AI 表格读取能力，如 `dws aitable record query`）。
2. 多项目时按 `合同金额` 从大到小排序后再写入 `projects`。**合同金额仅用于排序，绝不写进 HTML/Markdown 数据、不展示给客户、不进 Webhook。**
3. 「风险 · 关注」可结合底表的 `风险级别`、`是否超期` 佐证，但内容仍须来自周报正文，不得凭空编造。
4. 匹配不到项目时，按周报正文照常生成，不猜测金额或状态。
...
- `dissatisfactionOptions`：不满意原因快捷选项数组，**固定为四项**：`交付质量不满意`、`交付进度不满意`、`沟通响应不及时`、`其他`。
